...
# Build AWE Audio HAL using the same configuration but link with awepal
ifeq ($(TARGET_USES_GY), true)
include $(CLEAR_VARS)

LOCAL_MODULE := audio.primary.$(TARGET_BOARD_PLATFORM).awe
LOCAL_HEADER_LIBRARIES := libawepal_headers libhardware_headers qti_audio_kernel_uapi
LOCAL_EXPORT_C_INCLUDE_DIRS      := $(LOCAL_PATH)/audio_extn
LOCAL_CFLAGS += -Wno-macro-redefined
LOCAL_CFLAGS += -DSOUND_TRIGGER_PLATFORM_NAME=$(TARGET_BOARD_PLATFORM)
LOCAL_CFLAGS += -D_GNU_SOURCE
...
LOCAL_CFLAGS += -DAUDIO_FRAMEWORK_AWE
LOCAL_MODULE_RELATIVE_PATH := hw
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE_OWNER := qti
LOCAL_VENDOR_MODULE := true
